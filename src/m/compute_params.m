...
function compute_params(folder)
    
    HRF = load([folder '/Deconv_Detrend_4DVolume_hrf.mat']);

    nii = load_nii([folder '/Deconv_Detrend_4DVolume_Olrm_FWHM.nii']);
    FWHM = nii.img;
    ...
    nii.img = Time2dip;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_Time2dip.nii'])
    nii.img = DipHeight;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_DipHeight.nii'])
    nii.img = DipIntegral;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_DipIntegral.nii'])
    nii.img = PeakIntegral;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_PeakIntegral.nii'])
    nii.img = Time2trough;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_Time2trough.nii'])
    nii.img = TroughHeight;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_TroughHeight.nii'])
    nii.img = TroughIntegral;
    save_nii(nii,[folder '/Deconv_Detrend_4DVolume_TroughIntegral.nii'])